...
const navItems: { page: Page; label: string; icon: React.ComponentType<{ size?: number; className?: string }> }[] = [
  ...
  { page: 'daily', label: 'Daily Page', icon: CalendarDays },
  { page: 'weekly', label: 'Weekly Plan', icon: CalendarRange },
  { page: 'monthly', label: 'Monthly View', icon: BookOpen },
  { page: 'roles', label: 'Roles & Goals', icon: Target },
  { page: 'mission', label: 'Mission & Values', icon: Heart },
];

export function Sidebar({ currentPage, onNavigate, collapsed, onToggleCollapse }: SidebarProps) {
  return (
    <aside
      className={`flex flex-col h-screen bg-[#1e3a5f] text-white transition-all duration-300 shrink-0 ${
        collapsed ? 'w-16' : 'w-56'
      }`}
    >
      {/* Logo */}
